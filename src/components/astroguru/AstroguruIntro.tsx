...
import {
  ASTROGURU_HIGHLIGHTS,
  ASTROGURU_INTRO,
} from "@/content/astroguru";
import {
  easeOutExpo,
  fadeUp,
  staggerContainer,
...
const ICONS: Record<
  (typeof ASTROGURU_HIGHLIGHTS)[number]["icon"],
  LucideIcon
> = {
  heart: Heart,
  chip: Cpu,
...
export function AstroguruIntro() {
  return (
    <section aria-label="eAstroGuru introduction">
      ...
      <div className="bg-[#F7F5F2] py-10 sm:py-12 lg:py-14">
        <m.ul
          className="mx-auto grid max-w-[1200px] gap-4 px-4 sm:grid-cols-2 sm:gap-5 sm:px-6 lg:grid-cols-4 lg:px-8 xl:max-w-[1320px] xl:px-16 2xl:max-w-[1440px] 2xl:px-24"
          variants={staggerContainer}
          initial="hidden"
          whileInView="visible"
          viewport={viewportOnce}
        >
          {ASTROGURU_HIGHLIGHTS.map((item) => {
